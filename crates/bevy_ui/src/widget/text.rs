...
use bevy_text::{DrawableText, Font, FontAtlasSet, TextStyle, GlyphLayout};
...
pub fn text_layout_system(
    mut queued_text: Local<QueuedText>,
    fonts: Res<Assets<Font>>,
    mut query: Query<(Entity, Changed<Text>, &mut GlyphLayout)>,
    mut text_query: Query<(&Text, &mut GlyphLayout)>,
) {
    // layout queued text
    let mut new_queued_text = Vec::new();
    for entity in queued_text.entities.drain(..) {
        if let Ok(mut result) = text_query.entity(entity) {
            if let Some((text, mut layout)) = result.get() {
                // .......
                if let Some(font) = fonts.get(&text.font) {
                    let glyphs = font.build_relative_layout(&text.value, text.style.font_size);
    ...
    // layout changed text
    for (entity, text, mut layout) in &mut query.iter() {
        // .......
        if let Some(font) = fonts.get(&text.font) {
            let glyphs = font.build_relative_layout(&text.value, text.style.font_size);
            layout.glyphs = glyphs;
...
pub fn text_atlas_system(
    mut queued_text: Local<QueuedText>,
    mut textures: ResMut<Assets<Texture>>,
    fonts: Res<Assets<Font>>,
    mut font_atlas_sets: ResMut<Assets<FontAtlasSet>>,
    mut texture_atlases: ResMut<Assets<TextureAtlas>>,
    mut changed_text_query: Query<(Entity, Changed<Text>, &mut CalculatedSize, &GlyphLayout)>,
    mut text_query: Query<(&Text, &mut CalculatedSize, &GlyphLayout)>,
) {
    // add queued text to atlases
    let mut new_queued_text = Vec::new();
    for entity in queued_text.entities.drain(..) {
        if let Ok(mut result) = text_query.entity(entity) {
            if let Some((text, mut calculated_size, glyph_layout)) = result.get() {
    ...
    for (entity, text, mut calculated_size, glyph_layout) in &mut changed_text_query.iter() {
        let font_atlases = font_atlas_sets
            .get_or_insert_with(text.font.id, || FontAtlasSet::new(text.font.clone_weak()));
        // TODO: this call results in one or more TextureAtlases, whose render resources are created in the RENDER_GRAPH_SYSTEMS
        // stage. That logic runs _before_ the DRAW stage, which means we cant call add_glyphs_to_atlas in the draw stage
...
pub fn draw_text_system(
    ...
    mut query: Query<(&mut Draw, &Text, &Node, &GlobalTransform, &GlyphLayout)>,
) {
    for (mut draw, text, node, global_transform, glyph_layout) in &mut query.iter() {
        if let Some(font) = fonts.get(&text.font) {
